...
public class TextMenu extends JMenu {
    ...
    public TextMenu(DrawPanel owner) {
        ...
        size.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                ...
                if (s != null && s.trim().length() > 0) {
                    try {
                        int newSize = Integer.parseInt(s.trim());
                        panel.getTextItem().setFontSize(newSize);
                        panel.repaint();
        ...
        lineSpacing.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                ...
                String s = JOptionPane.showInputDialog(panel, "What line spacing do you want to use?",
                        currentLineSpacing);
                if (s != null && s.trim().length() > 0) {
                    try {
                        double newLineSpacing = Double.parseDouble(s.trim());
                        panel.getTextItem().setLineHeightMultiplier(newLineSpacing);
                        panel.repaint();
                    } catch (NumberFormatException e) {
     ...
     * DrawPanel. (Sets the italic and bold checkboxes to unselected.) This method is
     * called by the main program when the user selects the "New" command, to make
     * sure that the menu state reflects the contents of the panel.
     */
    ...
    /**
     * Create a menu containing a list of all available fonts. (It turns out this can
     * be very messy, at least on Linux, but it does show the user what is available
     * and lets the user try everything!)
     */
    private JMenu makeFontNameSubmenu() {
        ...
        if (fonts.length <= 20) {
            ...
            // Too many items for one menu; divide them into several sub-sub-menus.
            char ch1 = 'A';
            char ch2 = 'A';
            JMenu m = new JMenu();
            int i = 0;
